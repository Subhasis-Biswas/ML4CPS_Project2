...
Using this above procedure we selected 8 features out of 56 total features in the socio-demographic category. Although we are missing out on information, we still can capture some important aspects. Even amongst them there were some trivial and obvious correlations, for example \textit{ISRD(max) vs ISRD(avg)}. Eliminating the trivial ones, we end up with four pairs of features to carry out the spatial analysis using SDM.
